...
function RequestsPage() {
  ...
  useEffect(() => { if (familyId) callServer(listJoinRequests, { familyId }).then(r => setRows(r.requests)); }, [familyId]);
    ...
    <div>
      ...
      <Card className="mt-6 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 && <tr><td colSpan={4} className="p-6 text-center text-muted-foreground">So'rovlar yo'q</td></tr>}
            {rows.map(r => (
